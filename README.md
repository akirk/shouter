...
After Gutenberg accepts a sync request, `shouter_respond_to_wp_sync_requests()` runs on `rest_post_dispatch`. It ignores requests from the bot client itself, decodes each user-authored updateV2 payload with `gutenberg_yjs_decode_update_v2()`, and applies the decoded structs to a lightweight room-state index stored in `_shouter_room_state`. This state tracks block Y.Map items, paragraph attributes, `attributes.content` Y.Text items, root `document.content`, text-item origins, and which source paragraphs have already been shouted.

Paragraph completion is detected structurally. Shouter watches for a new empty top-level `core/paragraph` block whose Yjs origin points at a known non-empty paragraph block. That is the pattern Gutenberg creates when the user finishes a paragraph and creates the following empty paragraph. When Shouter sees that pattern, it reads the source paragraph text from the reconstructed room state, transforms it with `shouter_shout_text()`, and deduplicates the source block/text pair so the same paragraph is not shouted repeatedly.

The shouted text transform is deliberately simple: it uppercases the text and replaces ASCII punctuation with exclamation marks. For example, `Hello, world?` becomes `HELLO! WORLD!`. The inserted paragraph is plain text; formatting from the source paragraph is not preserved.

The actual mutation is generated by `includes/gutenberg-yjs-update-v2.php`, a small PHP library for the subset of Yjs updateV2 needed here. It is intentionally plugin-neutral: Shouter supplies the room state and bot identity, while the library only knows how to decode updateV2 structs and encode Gutenberg paragraph updates. The low-level varint, RLE, JSON-any, and content-length behavior comes from `yjs/yjs-php`; the library writes the exact updateV2 stream layout that Gutenberg's Yjs consumer expects. Shouter calls `gutenberg_yjs_encode_paragraph_insert_after_update_v2()` with the bot client ID, the current bot clock, the source paragraph's Yjs item ID as the left origin, and the following empty paragraph as the right origin. The generated update inserts a Gutenberg paragraph block map with `name: core/paragraph`, `isValid: true`, `attributes.content` as a Y.Text, `attributes.dropCap: false`, `innerBlocks` as a Y.Array, and a generated Gutenberg `clientId`.
