 ...
 */
final class YNestedArray implements \ArrayAccess, \Countable, \IteratorAggregate
{
    /**
    ...
    {
        return $this->doc->createDocFromSnapshot($snapshot)->nestedArrayValue($this->idKey);
    }
    ...
    public function getSharedType(int $index): self|YNestedMap|YNestedText|YNestedXmlFragment|YXmlElement|YXmlText|YXmlHook|null
    {
        return $this->doc->sharedTypeInNestedArray($this->idKey, $index);
    }

    public function getArray(int $index): ?self
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a nested YArray.', $index));
    }
    ...
    public function getMap(int $index): ?YNestedMap
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a nested YMap.', $index));
    }
    ...
    public function getText(int $index): ?YNestedText
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a nested YText.', $index));
    }
    ...
    public function getXmlElement(int $index): ?YXmlElement
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a YXmlElement.', $index));
    }
    ...
    public function getXmlText(int $index): ?YXmlText
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a YXmlText.', $index));
    }
    ...
    public function getXmlHook(int $index): ?YXmlHook
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a YXmlHook.', $index));
    }
    ...
    public function getXmlFragment(int $index): ?YNestedXmlFragment
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a YXmlFragment.', $index));
    }
    ...
    public function getSubdoc(int $index): ?YSubdoc
    {
        ...
        throw new \UnexpectedValueException(sprintf('YNestedArray item at index %d is not a subdoc.', $index));
    }
    ...
    public function offsetSet(mixed $offset, mixed $value): void
    {
        ...
        if ($index < 0 || $index > $count) {
            throw new \InvalidArgumentException('YNestedArray offset assignment index is out of bounds.');
        }
    ...
    public function set(int $index, mixed $value): void
    {
        $count = count($this->currentValues());
        if ($index < 0 || $index >= $count) {
            throw new \InvalidArgumentException('YNestedArray set index is out of bounds.');
        }
    ...
    public function replace(int $index, mixed $value): mixed
    {
        $count = count($this->currentValues());
        if ($index < 0 || $index >= $count) {
            throw new \InvalidArgumentException('YNestedArray replace index is out of bounds.');
        }
    ...
    {
        $this->doc->insertNestedArray($this->idKey, $index, $values);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    }
    ...
    {
        $this->doc->insertNestedArrayBinary($this->idKey, $index, $bytes);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    }
    ...
    public function insertBinaries(int $index, array $binaries): void
    {
        ...
        foreach ($binaries as $bytes) {
            if (! is_string($bytes)) {
                throw new \InvalidArgumentException('YNestedArray insertBinaries only supports binary strings.');
            }
        }

        $this->doc->transact(function () use ($index, $binaries): void {
            foreach (array_values($binaries) as $offset => $bytes) {
                $this->doc->insertNestedArrayBinary($this->idKey, $index + $offset, $bytes);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    }
    ...
    {
        $subdoc = $this->doc->insertNestedArraySubdoc($this->idKey, $index, $guid, $opts);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $this->assertInsertRange($index);
        $specs = self::normalizeSubdocSpecs($subdocs, 'YNestedArray insertSubdocs expects each subdoc to be a GUID string or array with a string guid.');
        $inserted = [];

        $this->doc->transact(function () use ($index, $specs, &$inserted): void {
            foreach ($specs as $offset => $spec) {
                $inserted[] = $this->doc->insertNestedArraySubdoc($this->idKey, $index + $offset, $spec['guid'], $spec['opts']);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    public function insertArray(int $index): self
    {
        $array = $this->doc->insertNestedArrayInNestedArray($this->idKey, $index);
        $this->values = $this->doc->nestedArrayValue($this->idKey);

        return $array;
    }

    public function appendArray(): self
    {
        return $this->insertArray($this->length());
    }

    public function prependArray(): self
    {
    ...
    {
        $this->assertInsertRange($index);
        $this->assertNonNegativeInsertCount($count, 'YNestedArray nested array insertion count must be non-negative.');

        $arrays = [];
        $this->doc->transact(function () use ($index, $count, &$arrays): void {
            for ($offset = 0; $offset < $count; $offset++) {
                $arrays[] = $this->doc->insertNestedArrayInNestedArray($this->idKey, $index + $offset);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $map = $this->doc->insertNestedMapInNestedArray($this->idKey, $index);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $this->assertInsertRange($index);
        $this->assertNonNegativeInsertCount($count, 'YNestedArray nested map insertion count must be non-negative.');

        $maps = [];
        $this->doc->transact(function () use ($index, $count, &$maps): void {
            for ($offset = 0; $offset < $count; $offset++) {
                $maps[] = $this->doc->insertNestedMapInNestedArray($this->idKey, $index + $offset);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $text = $this->doc->insertNestedTextInNestedArray($this->idKey, $index);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $this->assertInsertRange($index);
        $this->assertNonNegativeInsertCount($count, 'YNestedArray nested text insertion count must be non-negative.');

        $texts = [];
        $this->doc->transact(function () use ($index, $count, &$texts): void {
            for ($offset = 0; $offset < $count; $offset++) {
                $texts[] = $this->doc->insertNestedTextInNestedArray($this->idKey, $index + $offset);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $element = $this->doc->insertXmlElementInNestedArray($this->idKey, $index, $nodeName);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    public function insertXmlElements(int $index, array $nodeNames): array
    {
        ...
        foreach ($nodeNames as $nodeName) {
            if (! is_string($nodeName)) {
                throw new \InvalidArgumentException('YNestedArray insertXmlElements only supports string node names.');
            }
        }
        ...
        $this->doc->transact(function () use ($index, $nodeNames, &$elements): void {
            foreach (array_values($nodeNames) as $offset => $nodeName) {
                $elements[] = $this->doc->insertXmlElementInNestedArray($this->idKey, $index + $offset, $nodeName);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $xmlText = $this->doc->insertXmlTextInNestedArray($this->idKey, $index, $text);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    public function insertXmlTexts(int $index, array $texts): array
    {
        ...
        foreach ($texts as $text) {
            if (! is_string($text)) {
                throw new \InvalidArgumentException('YNestedArray insertXmlTexts only supports string XML text content.');
            }
        }
        ...
        $this->doc->transact(function () use ($index, $texts, &$xmlTexts): void {
            foreach (array_values($texts) as $offset => $text) {
                $xmlTexts[] = $this->doc->insertXmlTextInNestedArray($this->idKey, $index + $offset, $text);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $hook = $this->doc->insertXmlHookInNestedArray($this->idKey, $index, $hookName);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    public function insertXmlHooks(int $index, array $hookNames): array
    {
        ...
        foreach ($hookNames as $hookName) {
            if (! is_string($hookName)) {
                throw new \InvalidArgumentException('YNestedArray insertXmlHooks only supports string hook names.');
            }
        }
        ...
        $this->doc->transact(function () use ($index, $hookNames, &$hooks): void {
            foreach (array_values($hookNames) as $offset => $hookName) {
                $hooks[] = $this->doc->insertXmlHookInNestedArray($this->idKey, $index + $offset, $hookName);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $fragment = $this->doc->insertXmlFragmentInNestedArray($this->idKey, $index);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    public function insertXmlFragments(int $index, int $count): array
    {
        $this->assertInsertRange($index);
        if ($count < 0) {
            throw new \InvalidArgumentException('YNestedArray XML fragment insertion count must be non-negative.');
        }

        $fragments = [];
        $this->doc->transact(function () use ($index, $count, &$fragments): void {
            for ($offset = 0; $offset < $count; $offset++) {
                $fragments[] = $this->doc->insertXmlFragmentInNestedArray($this->idKey, $index + $offset);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    ...
    {
        $this->doc->deleteNestedArray($this->idKey, $index, $length);
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    }
    ...
    {
        $indexes = self::normalizeDeleteIndexes($indexes, $this->length(), 'YNestedArray deleteAll only supports valid integer indexes.');

        $this->doc->transact(function () use ($indexes): void {
            foreach ($indexes as $index) {
                $this->doc->deleteNestedArray($this->idKey, $index, 1);
            }
        });
        $this->values = $this->doc->nestedArrayValue($this->idKey);
    }
    ...
    {
        return $this->doc->observeNestedType($this->idKey, 'array', $observer);
    }
    ...
    {
        return $this->doc->observeNestedTypeOnce($this->idKey, 'array', $observer);
    }
    ...
    {
        if (! is_int($offset)) {
            throw new \InvalidArgumentException('YNestedArray offsets must be integers.');
        }
    ...
    {
        return $this->doc->nestedArrayValue($this->idKey);
    }

    private function assertInsertRange(int $index): void
    {
        if ($index < 0 || $index > $this->length()) {
            throw new \InvalidArgumentException('YNestedArray insert index is out of bounds.');
        }
    }
