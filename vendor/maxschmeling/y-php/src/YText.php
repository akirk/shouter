...
final class YText implements \Countable, \Stringable
{
    ...
    {
        return $this->doc->textValue($this->name);
    }

    public function toStringSnapshot(Snapshot $snapshot): string
    {
        return $this->doc->createDocFromSnapshot($snapshot)->getText($this->name)->toString();
    }
    ...
    {
        $this->doc->setTextAttribute($this->name, $key, $value);
    }
    ...
    public function setAttributes(array $attributes): void
    {
        $this->doc->transact(function () use ($attributes): void {
            foreach ($attributes as $key => $value) {
                $this->doc->setTextAttribute($this->name, (string) $key, $value);
            }
        });
    }
    ...
    {
        return $this->doc->textAttribute($this->name, $key);
    }

    public function getAttributeSnapshot(string $key, Snapshot $snapshot): mixed
    {
        return $this->doc->createDocFromSnapshot($snapshot)->getText($this->name)->getAttribute($key);
    }

    public function hasAttribute(string $key): bool
    {
        return $this->doc->textHasAttribute($this->name, $key);
    }

    public function hasAttributeSnapshot(string $key, Snapshot $snapshot): bool
    {
        return $this->doc->createDocFromSnapshot($snapshot)->getText($this->name)->hasAttribute($key);
    }
    ...
    {
        return $this->doc->textAttributes($this->name);
    }
    ...
    {
        return $this->doc->createDocFromSnapshot($snapshot)->getText($this->name)->getAttributes();
    }

    public function removeAttribute(string $key): void
    {
        $this->doc->removeTextAttribute($this->name, $key);
    }
    ...
    {
        return $this->doc->textDelta($this->name);
    }
    ...
    {
        return $this->doc->createDocFromSnapshot($snapshot)->getText($this->name)->toDelta();
    }
    ...
    public function insert(int $index, string $text, array $attributes = []): void
    {
        ...
        $this->doc->transact(function () use ($index, $text, $attributes, $clearAttributes): void {
            $this->doc->insertText($this->name, $index, $text, $attributes);
            if ($text !== '' && $clearAttributes !== []) {
                $this->doc->formatText($this->name, $index, self::utf16CodeUnitLength($text), $clearAttributes);
            }
        });

        $this->value = $this->doc->getText($this->name)->toString();
    }
    ...
    public function insertEmbed(int $index, mixed $embed, array $attributes = []): void
    {
        ...
        $this->doc->transact(function () use ($index, $embed, $attributes, $clearAttributes): void {
            $this->doc->insertTextEmbed($this->name, $index, $embed, $attributes);
            if ($clearAttributes !== []) {
                $this->doc->formatText($this->name, $index, 1, $clearAttributes);
            }
        });
    ...
    {
        $this->doc->deleteText($this->name, $index, $length);
        $this->value = $this->doc->getText($this->name)->toString();
    }
    ...
    {
        return $this->doc->createRelativePosition($this->name, $index, $assoc, 'text');
    }
    ...
    {
        $this->doc->formatText($this->name, $index, $length, $attributes);
        $this->value = $this->doc->getText($this->name)->toString();
    }
    ...
    {
        return $this->doc->observe($this->name, $observer, 'text');
    }
    ...
    {
        return $this->doc->observeOnce($this->name, $observer, 'text');
    }
    ...
    {
        return $this->doc->observeSharedTypeDeep($this->name, 'text', function (array $events, YDoc $doc, array $transaction) use ($observer): void {
